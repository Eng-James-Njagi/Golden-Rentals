...
export async function POST(request) {
  try {
    ...
    try {
      ...
      // Delete any images that made it to storage
      const imageFolder = `images/${listingId}`;
      const videoFolder = `videos/${listingId}`;

      const { data: imageList } = await supabase.storage.from(BUCKET).list(imageFolder);
      const { data: videoList } = await supabase.storage.from(BUCKET).list(videoFolder);

      const toDelete = [
        ...(imageList ?? []).map(f => `${imageFolder}/${f.name}`),
        ...(videoList ?? []).map(f => `${videoFolder}/${f.name}`),
      ];
