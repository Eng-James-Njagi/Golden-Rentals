...
AS $$
  ...
  filtered_ids AS (
    ...
    WHERE
      ...
      AND (p_type_ids      IS NULL OR pl.property_type_id  = ANY(p_type_ids))
      AND (p_rent_duration IS NULL OR pl.rent_duration     = p_rent_duration)
      AND (p_property_interior IS NULL OR LOWER(pl.property_interior) = LOWER(p_property_interior))
      AND (
        p_price_range IS NULL OR
        CASE p_price_range
          WHEN 'below_2000'  THEN pl.property_price::numeric <= 2000
          WHEN '2000_4000'   THEN pl.property_price::numeric > 2000  AND pl.property_price::numeric <= 4000
          WHEN '5000_8000'   THEN pl.property_price::numeric > 5000  AND pl.property_price::numeric <= 8000
          WHEN '9000_12000'  THEN pl.property_price::numeric > 9000  AND pl.property_price::numeric <= 12000
          WHEN '13000_20000' THEN pl.property_price::numeric > 13000 AND pl.property_price::numeric <= 20000
  ...
  review_stats AS (
    ...
    WHERE lr.listing_id IN (SELECT listing_id FROM filtered_ids)
